...
export class Channel implements IStorable, ISerializable<IChannel, Channel> {
  ...
  constructor(public path: string, name: string = null) {
    ...
    this.name = name || Path.basename(path);
  ...
  private guessHapifyFolder(): string {
    const existingPath = Channel.hapifyFolders.find((path) =>
      this.storageService.exists([this.path, path]),
    );
    return existingPath || Channel.hapifyFolders[0];
  }
  ...
  guessProjectIdOrPath() {
    if (
      !Project.isRemoteId(this.config.project) &&
      !Path.isAbsolute(this.config.project)
    ) {
