...
export abstract class Parser<T> {
	...
	constructor(private input: VersionedObject) {
		Container.get(VersionService).ensureVersionIsSupported(this.getScope(), input.version);
		this.initWorker();
	}
	...
	protected abstract getWorkersMap(): { [key: string]: new () => VersionedObjectParser<T> };
}
