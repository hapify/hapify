...
describe('new command', () => {
  ...
  it('success by url (x2) with preset', async () => {
    ...
    expect(
      sandbox.fileExists(['boilerplate-hapijs', 'hapify.json']),
    ).to.be.true();

    expect(sandbox.dirExists(['boilerplate-ngx-components'])).to.be.true();
    expect(
      sandbox.fileExists(['boilerplate-ngx-components', 'hapify.json']),
    ).to.be.true();
    expect(
      sandbox.fileExists(['boilerplate-ngx-components', 'hapify-models.json']),
    ).to.be.false();

    const hapifyJSON1 = sandbox.getJSONFileContent<IConfig>([
      'boilerplate-hapijs',
      'hapify.json',
    ]);
    const hapifyJSON2 = sandbox.getJSONFileContent<IConfig>([
    ...
    expect(
      sandbox.getPath(['boilerplate-hapijs', hapifyJSON1.project]),
    ).to.equal(
      sandbox.getPath(['boilerplate-ngx-components', hapifyJSON2.project]),
    );

    const hapifyModelsJSON = sandbox.getJSONFileContent<IStorableCompactProject>(
      ['boilerplate-hapijs', hapifyJSON1.project],
    );
    expect(hapifyModelsJSON.models.length).to.equal(1);
    expect(
      hapifyModelsJSON.models.some((m) => m.name.toLowerCase() === 'user'),
    ).to.be.true();
  });
  it('success with presets', async () => {
    ...
    expect(
      hapifyModelsJSON.models.some((m) => m.name.toLowerCase() === 'place'),
    ).to.be.true();
  });
