...
export type FieldValueType<T> = T extends 'entity'
  ? string
  : T extends 'enum'
  ? string[]
  : null;
...
export type ExplicitFieldsFilterFunction = (
  callback:
    | ((value: ExplicitField, index: number, array: ExplicitField[]) => boolean)
    | null,
) => ExplicitField[];
 ...
 */
export type ExplicitModelDependenciesFilter = (
  filter?: (field: ExplicitField) => boolean,
  excludeSelf?: boolean,
) => ExplicitDeepModel[];
...
export type ExplicitModelAccessFilter = (
  func: (a: ExplicitAccesses) => boolean | null,
) => ExplicitAccesses[];
...
export type GenerationContext =
  | SingleModelGenerationContext
  | MultipleModelsGenerationContext;
