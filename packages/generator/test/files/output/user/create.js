...
const handler = async (request, h) => {
	...
	payload.role = typeof request.payload.role !== 'undefined' ? request.payload.role : 'user';
	payload.banned = typeof request.payload.banned !== 'undefined' ? request.payload.banned : false;
	payload.premium_forced = typeof request.payload.premium_forced !== 'undefined' ? request.payload.premium_forced : false;
	...
	let result;
	try {
		result = await request.server.db.collection('user').insertOne(payload, options);
	} catch (e) {
		// Handle duplicated key for unique indexes
		return e.name === 'MongoError' && e.code === 11000 ? Boom.conflict('Duplicate key') : Boom.boomify(e);
	}
