...
out += \``;
  ...
  protected replace(
    ...
    replace: string | ReplacementCallback,
  ): this {
    let patternOffset = 0;

    this.factory.template = this.factory.template.replace(
      regexp,
      (...params: string[]) => {
