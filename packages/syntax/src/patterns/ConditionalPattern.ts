...
const Replacements: Replacement[] = [
  ...
  {
    search: ['email', 'tSe'],
    replace: "(i.type === 'string' && i.subtype === 'email')",
  },
  {
    search: ['password', 'tSp'],
  ...
  {
    search: ['url', 'tSu'],
    replace: "(i.type === 'string' && i.subtype === 'url')",
  },
  {
    search: ['text', 'tSt'],
    replace: "(i.type === 'string' && i.subtype === 'text')",
  },
  {
    search: ['richText', 'rich', 'tSr'],
    replace: "(i.type === 'string' && i.subtype === 'rich')",
  },
  ...
  {
    search: ['float', 'tNf'],
    replace: "(i.type === 'number' && i.subtype === 'float')",
  },
  {
    search: ['latitude', 'tNt'],
  ...
  {
    search: ['date', 'tDd'],
    replace: "(i.type === 'datetime' && i.subtype === 'date')",
  },
  {
    search: ['time', 'tDt'],
    replace: "(i.type === 'datetime' && i.subtype === 'time')",
  },
  ...
  {
    search: ['oneOne', 'tEoo'],
    replace: "(i.type === 'entity' && i.subtype === 'oneOne')",
  },
  {
    search: ['oneMany', 'tEom'],
    replace: "(i.type === 'entity' && i.subtype === 'oneMany')",
  },
  {
    search: ['manyOne', 'tEmo'],
    replace: "(i.type === 'entity' && i.subtype === 'manyOne')",
  },
  {
    search: ['manyMany', 'tEmm'],
    replace: "(i.type === 'entity' && i.subtype === 'manyMany')",
  },
  ...
  {
    search: ['image', 'tFi'],
    replace: "(i.type === 'file' && i.subtype === 'image')",
  },
  {
    search: ['video', 'tFv'],
    replace: "(i.type === 'file' && i.subtype === 'video')",
  },
  {
    search: ['audio', 'tFa'],
    replace: "(i.type === 'file' && i.subtype === 'audio')",
  },
  {
    search: ['document', 'tFd'],
    replace: "(i.type === 'file' && i.subtype === 'document')",
  },
  ...
  {
    search: ['mainlyInternal', 'pMIn'],
    replace: 'i.properties.mainlyInternal',
  },
  { search: ['isGeolocated', 'pGeo'], replace: 'i.properties.isGeolocated' },
  {
    search: ['isGeoSearchable', 'pGSe'],
    replace: 'i.properties.isGeoSearchable',
  },
...
export class ConditionalPattern extends BasePattern {
  ...
  execute(): void {
    this.replace(IfPattern, (match, statement, count, variable, condition) => {
      const jsCondition = this.condition(
        count,
      ...
      .replace(
        ElseIfPattern,
        (match, statement, count, variable, condition) => {
          const jsCondition = this.condition(
            count,
            this.variable(variable),
            this.tester(condition),
    ...
    if (typeof Testers[trimmed] === 'undefined') {
      const condition = trimmed
        .replace(Condition, (match) => {
          const replacement = Replacements.find((l) =>
            l.search.includes(match),
          );
          if (!replacement) {
            throw new InternalError(
              `[ConditionalPattern.tester] Cannot find condition replacement for match: ${match} (in :${trimmed})`,
            );
          }
  ...
  protected variable(variable: string): string {
    ...
    if (['R', 'ReferencedIn', 'RefModels'].includes(variable))
    ...
    if (['Ac', 'CreateAccess'].includes(variable))
      return 'root.accesses.create';
    if (['Ar', 'ReadAccess'].includes(variable)) return 'root.accesses.read';
    if (['Au', 'UpdateAccess'].includes(variable))
      return 'root.accesses.update';
    if (['Ad', 'RemoveAccess'].includes(variable))
      return 'root.accesses.remove';
    if (['As', 'SearchAccess'].includes(variable))
