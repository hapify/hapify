...
const InputConditional = readFileSync(
  `${__dirname}/templates/error-conditional.hpf`,
  'utf8',
);
const InputIteration = readFileSync(
  `${__dirname}/templates/error-iteration.hpf`,
  'utf8',
);
const InputInterpolation = readFileSync(
  `${__dirname}/templates/error-interpolation.hpf`,
  'utf8',
);
const InputTimeout = readFileSync(
  `${__dirname}/templates/error-timeout.hpf`,
  'utf8',
);
const InputRequire = readFileSync(
  `${__dirname}/templates/error-require.hpf`,
  'utf8',
);
const InputImport = readFileSync(
  `${__dirname}/templates/error-import.hpf`,
  'utf8',
);
const InputGlobal = readFileSync(
  `${__dirname}/templates/error-global.hpf`,
  'utf8',
);
const InputReturn = readFileSync(
  `${__dirname}/templates/error-return.hpf`,
  'utf8',
);
...
describe('errors', () => {
  it('run', () => {
    ...
    expect(() => (<any>HapifySyntax.run)(Simple)).to.throw(ArgumentsError);
    expect(() => (<any>HapifySyntax.run)(Simple, null)).to.throw(
      ArgumentsError,
    );
    expect(() => (<any>HapifySyntax.run)(Simple, true)).to.throw(
      ArgumentsError,
    );
    expect(() => (<any>HapifySyntax.run)(Simple, 3)).to.throw(ArgumentsError);
    expect(() => (<any>HapifySyntax.run)(Simple, 'string')).to.throw(
      ArgumentsError,
    );

    expect(() => (<any>HapifySyntax.run)(undefined, Model)).to.throw(
      ArgumentsError,
    );
    expect(() => (<any>HapifySyntax.run)(null, Model)).to.throw(ArgumentsError);
    expect(() => (<any>HapifySyntax.run)(false, Model)).to.throw(
      ArgumentsError,
    );
  ...
  it('Check reverse lineColumn error for conditional', () => {
    ...
    try {
      ...
      expect(err.details).to.be.equal(
        'Error: rrgrfbfb is not defined. Line: 16, Column: 13',
      );
    }
  });
  ...
  it('Check reverse lineColumn error for iteration', () => {
    ...
    try {
      ...
      expect(err.columnNumber).to.be.a.number();
      expect(err.details).to.be.equal(
        'Error: T is not defined. Line: 10, Column: 1',
      );
    }
  });
  ...
  it('Check reverse lineColumn error for name interpolation', () => {
    ...
    try {
      ...
      expect(err.columnNumber).to.be.a.number();
      expect(err.details).to.be.equal(
        'Error: g is not defined. Line: 20, Column: 5',
      );
    }
  });
  ...
  it('Cannot use require', () => {
    ...
    try {
      ...
      expect(err.columnNumber).to.be.a.number();
      expect(err.details).to.be.equal(
        'Error: require is not defined. Line: 5, Column: 1',
      );
    }
  });
  ...
  it('Cannot use import', () => {
    ...
    try {
      ...
      expect(err.details).to.be.equal(
        'Error: Cannot use import statement outside a module. Line: null, Column: null',
      );
    }
  });
